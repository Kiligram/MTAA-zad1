...
rx_invalid = re.compile("99999999999")
rx_invalid2 = re.compile("99999999999")
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        if rx_invalid.search(contact) or rx_invalid2.search(contact):
            if fromm in registrar:
                del registrar[fromm]
            self.sendResponse("488 Nie je tu pripustny")
            return
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 VELMI OK")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            self.sendResponse("500 Chyba na servere")
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Nie je tu pripustne")
        else:
            self.sendResponse("500 Chyba na servere")
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                global prev_code
                if data[0] == "SIP/2.0 200 Ok" and data[5] == "CSeq: 20 INVITE" and prev_code != "SIP/2.0 200 Ok":
                    journal.write(time.strftime("%H:%M:%S", time.localtime()))
                    journal.write(": >>> ")
                    journal.write(f"Call {self.getOrigin()} <---> {self.getDestination()} has been accepted \n")
                elif data[0] == "SIP/2.0 200 Ok" and data[5] == "CSeq: 20 CANCEL":
                    journal.write(time.strftime("%H:%M:%S", time.localtime()))
                    journal.write(": >>> ")
                    journal.write(f"Call {self.getOrigin()} <---> {self.getDestination()} has been canceled \n")
                elif data[0] == "SIP/2.0 603 Decline" and data[5] == "CSeq: 20 INVITE" and prev_code != "SIP/2.0 603 Decline":
                    journal.write(time.strftime("%H:%M:%S", time.localtime()))
                    journal.write(": >>> ")
                    journal.write(f"Call {self.getOrigin()} <---> {self.getDestination()} has been declined \n")
                journal.close()
                prev_code = data[0]
                text = "\r\n".join(data)
                socket.sendto(text.encode(), claddr)
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                journal.write(time.strftime("%H:%M:%S", time.localtime()))
                journal.write(": >>> ")
                journal.write(f"{self.getOrigin()} is calling {self.getDestination()} \n")
                self.processInvite()
                ...
                journal.write(time.strftime("%H:%M:%S", time.localtime()))
                journal.write(": >>> ")
                journal.write(f"Call {self.getOrigin()} <---> {self.getDestination()} has ended \n")
                ...
                self.sendResponse("200 VELMI 0K")
